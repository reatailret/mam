	...
	return {
		...
		async resolveId( id )
		{
			...
			if(id.indexOf(virtualModuleId) >=0 ){
				...
				const orgId = id.split(virtualModuleRootId)[1];
				ROOT_CLASS = orgId
				...
				id.indexOf(resolvedVirtualModuleId) >= 0)
			{
				...
				for( const mol_class of [ ...mol_classes ] )
				{
					...
					for( const { path: file, ext } of files )
					{
						...
						{
							const f = (/\.ts$/.test( file ) || /\.view\.tree$/.test( file )) ? ( resolvedVirtualModuleId + file  ) : file
							...
							//const { exports:exportings2 } = await this.load( { id: f, resolveDependencies: true } )
							let code2 = ''
